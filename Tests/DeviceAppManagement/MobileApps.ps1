...
$newApps = (1..$numApps | ForEach-Object {
    New-MobileApps -webApp -displayName 'My new app' -publisher 'Test web app' -appUrl 'https://www.bing.com'
})
...
$searchedApps = Get-MobileApps -Filter "isof('microsoft.graph.webApp')"
$ids = $newApps.id
...
$app = $allApps[0]
$appCategory | New-MobileAppsCategoriesReferences -mobileAppId $app.id

# Get the referenced categories on this app
Write-Host "Getting the app with categories and assignments expanded..."
$app = $app | Get-MobileApps -Expand assignments, categories

# DELETE the reference
Write-Host "Removing the reference between the app and the category"
$appCategory | Remove-MobileAppsCategoriesReferences -mobileAppId $app.id
...
$firstPage = Get-MobileApps -Top ($allApps.Count / 3)
...
$success = $false
try {
    Get-MobileApps | Select-Object -First 1 | Out-Null
