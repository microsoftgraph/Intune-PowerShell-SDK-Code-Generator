...
$newApps = (1..$numApps | ForEach-Object {
    New-DeviceAppManagementMobileApps -webApp -displayName 'My new app' -publisher 'Test web app' -appUrl 'https://www.bing.com'
})
...
$searchedApps = Get-DeviceAppManagementMobileApps -Filter "isof('microsoft.graph.webApp')"
$ids = $newApps.id
...
$newApps | Update-DeviceAppManagementMobileApps -displayName $newAppName

# Batch GET apps (with PowerShell pipeline)
Write-Host "Retrieving the updated apps with the PowerShell pipeline..."
$updatedApps = $newApps | Get-DeviceAppManagementMobileApps -Select id, displayName
...
$updatedApps | Remove-DeviceAppManagementMobileApps

# SEARCH all app categories
Write-Host "Getting all app categories..."
Get-DeviceAppManagementMobileAppCategories | Out-Null

# Create a custom category
Write-Host "Creating a new app category..."
$appCategory = New-DeviceAppManagementMobileAppCategories -displayName 'Test Category'

# SEARCH all apps
Write-Host "Getting all apps..."
$allApps = Get-DeviceAppManagementMobileApps
...
$app = $allApps[0]
$appCategory | New-DeviceAppManagementMobileAppsCategoriesReferences -mobileAppId $app.id

# Get the referenced categories on this app
Write-Host "Getting the app with categories and assignments expanded..."
$app = $app | Get-DeviceAppManagementMobileApps -Expand assignments, categories

# DELETE the reference
Write-Host "Removing the reference between the app and the category"
$appCategory | Remove-DeviceAppManagementMobileAppsCategoriesReferences -mobileAppId $app.id

# DELETE the category
Write-Host "Deleting the category"
$appCategory | Remove-DeviceAppManagementMobileAppCategories

# Run some paging commands
Write-Host "Testing paging..."
$firstPage = Get-DeviceAppManagementMobileApps -Top ($allApps.Count / 3)
...
$success = $false
try {
    Get-DeviceAppManagementMobileApps | Select-Object -First 1 | Out-Null
